...
template<avl_request_type T>
int calc_factor(avl_node<T>* root, int right_d = 0, int left_d = 0) {
	int right_deep = right_d;
	int left_deep = left_d;
	if (root != nullptr) {
		if (root->left != nullptr) {
			++left_deep;
			left_deep += calc_factor(root->left, right_deep, left_deep);
		}
		if (root->right != nullptr) {
			--right_deep;
			right_deep += calc_factor(root->right, right_deep, left_deep);
		}
	}
